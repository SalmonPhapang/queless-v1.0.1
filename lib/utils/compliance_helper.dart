class ComplianceHelper {
  static const Map<String, Map<String, dynamic>> provinceRules = {
    'Western Cape': {
      ...
      'sunday_sales': false,
      'restrictions':
          'No alcohol sales after 9 PM on weekdays and Saturdays. No sales on Sundays.',
    },
    'Gauteng': {
      'weekday_cutoff': 22,
      'sunday_sales': true,
      'sunday_cutoff': 18,
      'restrictions': 'Alcohol sales: 10 PM weekdays/Saturdays, 6 PM Sundays.',
    },
    'KwaZulu-Natal': {
      'weekday_cutoff': 21,
      'sunday_sales': true,
      'sunday_cutoff': 18,
      'restrictions': 'Alcohol sales: 9 PM weekdays/Saturdays, 6 PM Sundays.',
    },
    'Eastern Cape': {
      'weekday_cutoff': 21,
      'sunday_sales': false,
  ...
  static bool canOrderAlcohol(String province) {
    ...
    if (now.weekday == DateTime.sunday) {
      if (!rules['sunday_sales']) return false;
      final sundayCutoff = rules['sunday_cutoff'] as int;
      if (now.hour >= sundayCutoff) return false;
    } else {
      final cutoffHour = rules['weekday_cutoff'] as int;
      if (now.hour >= cutoffHour) return false;
    }
  ...
  static String getRestrictionMessage(String province) {
    final rules = provinceRules[province];
    if (rules == null) {
      return 'Unable to determine restrictions for this province.';
    }
...
*   **Application** means the software program provided by the Company downloaded by You on any electronic device, named Queless.
*   **Company** (referred to as either "the Company", "We", "Us" or "Our" in this Agreement) refers to Queless.
